...
const Beams: FC<BeamsProps> = ({
  ...
  float getPos(vec3 pos) {
    vec3 noisePos =
      vec3(pos.x * 0., pos.y - uv.y, pos.z + time * uSpeed * 0.1) * uScale;
    float dist = distance(uv, uMouse);
    float mouseEff = (1.0 - smoothstep(0.0, 0.4, dist)) * 0.5;
    return cnoise(noisePos) + mouseEff; 
  }
...
const MergedPlanes = forwardRef<THREE.Mesh<THREE.BufferGeometry, THREE.ShaderMaterial>, { material: THREE.ShaderMaterial; width: number; count: number; height: number; }>(({ material, width, count, height }, ref) => {
    ...
    useFrame((_state, delta) => {
        if (mesh.current) {
            mesh.current.material.uniforms.time.value += 6.5 * delta;
            mesh.current.material.uniforms.uMouse.value.lerp(new THREE.Vector2(mouse.x * 0.5 + 0.5, mouse.y * 0.5 + 0.5), 0.1);
        }
    });
